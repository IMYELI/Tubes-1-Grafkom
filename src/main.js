...
function bufferRectangle(gl, x, y, width, height) {
  var x1 = x;
  var x2 = x + width;
  var y1 = y;
  var y2 = y + height;
...
function bufferRandomColor(gl, colorUniformLocation, opacity = 1) {
  gl.uniform4f(colorUniformLocation, Math.random(), Math.random(), Math.random(), opacity);
}

// Create <count> rectangle
function generateRectangle(gl, colorUniformLocation, count = 1) {
  var primitiveType = gl.TRIANGLES;
  var offset = 0;
  var shaderCount = 6;
  for (var i = 0; i < count; i++) {
    bufferRectangle(gl, randInt(300), randInt(300), randInt(300), randInt(300));
    bufferRandomColor(gl, colorUniformLocation);
...
async function main() {
  ...
  generateRectangle(gl, colorUniformLocation, 5);
}
